...
-/
import Amenability.FiniteSubcoalgebraTransferPrimal
...
ambient spaces.
-/
theorem mul_subcoalgebraDensitySubspace_le_primal
  ...
  have hsem :
    ...
    exact hs
  let Wamb : Submodule k H := F.carrier * Uamb
  let W : Submodule k FG := Wamb.comap FG.subtype
  have hWimage :
      ambientImage FG W = Wamb := by
  ...
  have hWFCt : W ≤ FCt := by
    ...
    rw [hWimage] at hx'
    change (x : H) ∈ F.carrier * Uamb at hx'
  ...
  have htransfer := finiteSubcoalgebra_transfer_ambient F Cfin U Damb
    hDambFC t hDamb hsem
  have hproductU : F.carrier * ambientImage Cfin.carrier U = Wamb := by
    rw [hUimage]
  rw [hproductU] at htransfer
  have hmulcarrier :
  ...
  have hWintersection :
    ...
    rw [← ambientImage_inf]
  have hdimFC :
      finrank k (F.carrier * Cfin.carrier) = finrank k FCt := by
    change finrank k (F.carrier * Camb) = finrank k FCt
    rw [← hFCtImage]
