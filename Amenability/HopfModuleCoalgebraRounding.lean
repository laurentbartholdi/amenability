...
subcoalgebra.
-/
theorem exists_finiteSubcoalgebra_action_ratio_le_internal
    (F : FiniteSubcoalgebra k H)
    (G : FiniteSubcoalgebra k M)
    (E : Submodule k G.carrier)
    (hE : E ≠ ⊥) :
    ∃ C : FiniteSubcoalgebra k M,
      C.carrier ≠ ⊥ ∧
        (finrank k (actionSubspace F.carrier C.carrier) : ℚ) /
            (finrank k C.carrier : ℚ) ≤
          (finrank k (actionSubspace F.carrier (ambientImage G.carrier E)) : ℚ) /
            (finrank k E : ℚ) := by
  let FG : Submodule k M := actionSubspace F.carrier G.carrier
  let : FiniteDimensional k FG := finiteDimensional_actionSubspace F.carrier G.carrier
  let FE : Submodule k FG :=
    (actionSubspace F.carrier (ambientImage G.carrier E)).comap FG.subtype
  ...
  have hFG : actionSubspace F.carrier (ambientImage G.carrier E) ≤ FG := by
    apply actionSubspace_mono_right F.carrier
    rintro x ⟨y, hy, rfl⟩
    exact y.2
  have hFEImage : ambientImage FG FE =
      actionSubspace F.carrier (ambientImage G.carrier E) :=
  ...
  let b : ℚ → ℚ := fun t =>
    finrank k
      (actionSubspace F.carrier (ambientImage G.carrier
  ...
  have hb_le : ∀ t : ℚ, 0 ≤ t → t ≤ 1 →
    ...
    intro t ht0 ht1
    have hle := actionSubspace_subcoalgebraDensitySubspace_le F G.carrier E t
    let FCt : Submodule k M := actionSubspace F.carrier (ambientImage G.carrier
      (subcoalgebraDensitySubspace G.carrier E t))
  ...
  · change
      (finrank k (actionSubspace F.carrier (ambientImage G.carrier Ct)) : ℚ) /
...
-/
theorem exists_finiteSubcoalgebra_action_ratio_le_of_le
    (F : FiniteSubcoalgebra k H)
    (E : Submodule k M)
    ...
    ∃ C : FiniteSubcoalgebra k M,
      C.carrier ≠ ⊥ ∧
        (finrank k (actionSubspace F.carrier C.carrier) : ℚ) /
            (finrank k C.carrier : ℚ) ≤
          (finrank k (actionSubspace F.carrier E) : ℚ) /
            (finrank k E : ℚ) := by
  ...
  obtain ⟨C, hC, hratio⟩ :=
    exists_finiteSubcoalgebra_action_ratio_le_internal F G Eint hEint
  refine ⟨C, hC, ?_⟩
...
/-- Global rounding for a cocommutative Hopf algebra acting on an arbitrary
module coalgebra.  The target coalgebra is not assumed cocommutative. -/
theorem exists_finiteSubcoalgebra_action_ratio_le
    (F : FiniteSubcoalgebra k H)
    (E : Submodule k M)
    [FiniteDimensional k E]
    (hE : E ≠ ⊥) :
  ...
  exact exists_finiteSubcoalgebra_action_ratio_le_of_le F E hE G hEG
