...
/-!
# Theorem E: elementary and subexponential amenability differ

This file formalizes the two examples collected as Theorem E in the
...
/-- Theorem E, characteristic-zero witness form: `SL` contains the Witt
...
/-- Theorem E: the elementary and subexponentially amenable classes are
distinct. -/
...
/-- Reference [3], packaged in precisely the form needed for Theorem E: over
...
axiom exists_psz_subexponential_not_elementary
    (p : ℕ) [Fact p.Prime] [CharP k p] :
    ∃ L : LieAlgebraObject k,
      IsSubexponentiallyAmenableLieObject (k := k) L ∧
        ¬ IsElementaryLieObject.{u, u, u} L

/-- The positive-characteristic case of Theorem E, deduced directly from
the cited PSZ input. -/
...
/-- Theorem E over an arbitrary field: `EL` and `SL` are distinct.  The
